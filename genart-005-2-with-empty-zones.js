...
const sketch = ({ width, height }) => {
  ...
  return ({ context, width, height, units }) => {
    ...
    const emptyZones = [
        { left: 78, top: 19, right: 85, bottom: 230 },
        { left: 85, top: 30, right: 160, bottom: 95 },
        { left: 36, top: 153, right: 80, bottom: 196 },
    ];
    ...
    points.forEach(data => {
      ...
      const y = lerp(marginTop, height - marginBottom, position[1]);

      if (emptyZones.some(zone => x > zone.left && x < zone.right && y > zone.top && y < zone.bottom)) {
        return; // skip points that fall within any of the empty zones
      }

      const triangle = () => {
        let h = Math.abs(rotation*20),
            w1 = Math.abs(rotation*2),
            h2 = Math.abs(rotation*2);
