...
const sketch = ({ width, height }) => {
  ...
  let countX = 50, countY = 20;
  let randoms = [];
  for (let i = 0; i < 8; i++) {
    randoms.push(random.getRandomSeed());
  }
  ...
  return ({ context, width, height, units }) => {
    ...
    const draw = (origin, w, h, opts) => {
      ...
      console.log(randoms[opts.index])
        random.setSeed(randoms[opts.index]);
    ...
    postcards.drawFourColumnsTowoRowws(draw, width, height);
