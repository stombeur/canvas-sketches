...
const sketch = ({ width, height }) => {
  ...
  const prepare = (origin, w, h) => {
    let hatch_spacing = h/300;
    let hatch_angle = 305;

    //plans = [];
    let boundsL = boundingbox.from([origin, postcards.reorigin([w, 0], origin), postcards.reorigin([w, h], origin), postcards.reorigin([0, h], origin)]);
    let roomsL = drawPlan(boundsL.center, w);
    ...
    hatchRegionL.regions.forEach(hh => {
      let x = hatch.inside(hh, hatch_angle, hatch_spacing);
      hatches.push(...x);
    })
