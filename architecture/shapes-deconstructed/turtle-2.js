...
import { DoubleCross, DoubleCrossBorder, HH, ITPLogo, RectangularBorder, TurtleShape } from '../shape';
...
const drawShape = (coords, width, height, card, thinLines, thickLines, split = false) => {   
    let center = [coords[0]+width/2, coords[1]+ height/2];
    let w = width/2;
    let h = height/2;
    let p_center = new point(... center);

    let sc = new TurtleShape(center, width / 20, 'RRRRRRRDDDLLDRRDDDDDDDLLLLLUUURRULLLDDDDDDDDDDDRRRRRUUUUULLLDDDRUURDDDLLLUUUUURRRRRDDDDDDDLLLLLLLUUUUUUUUUUUUURRRRRDDDLLDRRRUUUUULLUUURRULLLLLDDDRUURDDDLLLUUUUU');//'RRRRRRRDDDLLDRRDDDDDDDLLLLLDDDRRULLLDDDDDDDDDDDRRRRRUUUUULLLDDDRUURDDDLLLUUUUURRRRRDDDDDDDLLLLLLLUUUUUUUUUUUUURRRRRDDDLLDLLLUUUUULLUUURRULLLLLDDDRUURDDDLLLUUUUU'); 
  ...
  let hatchregions = sc.toClipRegion().copy([3,3]).subtract(sc_clip_split);
...
const sketch = ({ width, height }) => {
  ...
  return ({ context, width, height, units }) => {
    ...
    const draw = (origin, w, h, opts) => {
      ...
      drawShape(localOrigin, w/2, h, card, paths1, paths2);

      // draw split rigth
      let localOrigin2 = postcards.reorigin([w/2, 0], origin);
      drawShape(localOrigin2, w/2, h, card, paths1, paths2, true);
