...
const drawShape = (coords, width, height, card, thinLines, thickLines, shapegrid, split = false) => {   
    ...
    console.log(' flood fill shape ok ')
  ...
  let hatchregions = sc.toClipRegion().copy([1,-1]).subtract(sc_clip_split);
...
const sketch = ({ width, height }) => {
    ...
    cards.forEach(card => {
        ...
        card.grid = floodfill(rows, columns, {iterations: 1000, weights: { count2s: 0, countMores: 0, countAdjacent: 1 }});
