...
const drawShape = (coords, width, height, card) => {   
    ...
    let sc3 = boundingbox.fromTopleft([padleft+((totalwidth-columngap)/2+columngap),padtop+39], 18, 33);
    let sc4 = boundingbox.fromTopleft([padleft+24,padtop+72], 93, 88);


    let sc = new clipregion();
    let topleft_bb = boundingbox.fromTopleft([padleft, padtop], 69, 145.5);
    let topleft_bb_subtract = boundingbox.fromTopleft([padleft+38, padtop+103], 33, 25.5);
    sc.addRegion(topleft_bb.points);
    let sc_subtract = new clipregion();
    sc_subtract.addRegion(topleft_bb_subtract.points);
    sc = sc.subtract(sc_subtract);

    let bottomleft_bb = boundingbox.fromTopleft([padleft, padtop+145.5+25], 69, 38.5);
    sc.addRegion(bottomleft_bb.points);

    let topright_bb = boundingbox.fromTopleft([padleft+69+3, padtop], 69, 11);
    sc.addRegion(topright_bb.points);

    let middleright_bb = boundingbox.fromTopleft([padleft+69+3, padtop+36], 69, 61);
    sc.addRegion(middleright_bb.points);
  
    let bottomright_bb = boundingbox.fromTopleft([padleft+69+3, padtop+36+61+27], 69, 85);
    sc.addRegion(bottomright_bb.points);
