...
const drawShape = (coords, width, height, card) => {   
    ...
    let sc = boundingbox.fromWH(center, w, h);
    ...
    // sc_clip.toLines().forEach(l => {
    //     result.push(createLinePath(l));
    // });
    let sc_clip_split = sc_clip;
    lines.forEach(l => {
      sc_clip_split = sc_clip_split.split(l.line, card.lines_bb, 4, 0.2);
    });
    ...
    let sc_clip_lines = sc_clip.subtract(sc_clip_split);
    sc_clip_lines.toLines().forEach(l => {
        result.push(createLinePath(l));
    });
