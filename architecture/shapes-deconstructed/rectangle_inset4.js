...
const drawShape = (coords, width, height, card) => {   
    ...
    let topleft_bb = boundingbox.fromTopleft([padleft, padtop], 69, 73);
    sc.addRegion(topleft_bb.points);

    let bottomleft_bb = boundingbox.fromTopleft([padleft, padtop+73+32], 69, 103);
    sc.addRegion(bottomleft_bb.points);

    let topright_bb = boundingbox.fromTopleft([padleft+69+3, padtop], 69, 73);
    sc.addRegion(topright_bb.points);
  
    let bottomright_bb = boundingbox.fromTopleft([padleft+69+3, padtop+73+32], 69, 103);
