...
const drawShape = (coords, width, height, card) => {   
   ...
   let sc = new clipregion();
    let left_bb = boundingbox.fromTopleft([padleft, padtop], (totalwidth-columngap)/2, columnheight);
    sc.addRegion(left_bb.points);

    let right_bb = boundingbox.fromTopleft([padleft+(totalwidth-columngap)/2+columngap, padtop], (totalwidth-columngap)/2, columnheight);
    sc.addRegion(right_bb.points);
