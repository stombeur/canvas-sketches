...
const drawGemstone = (circle, light) => {
  ...
  let pointLight = light.o;
  drawLines.push(drawArc(pointLight, 2, 0, 360));
      ...
      for (let j = 0; j < 1; j++) {
        ...
        for (let i = 0; i < points.length; i++) {
          let d = poly.distanceBetween(points[i], pointLight);
          
          if (d < distanceToLight) { skipIndex = i; distanceToLight = d; }
          console.log(distanceToLight, skipIndex)
        }

        //console.log(pointLight, distanceToLight, skipIndex)
        drawLines.push(drawArc(points[skipIndex], 2, 0, 360));
        ...
        if (skipIndex>-1) {
          ...
          //drawLines.push(drawArc(points[inverseSkipIndex], 2, 0, 360));
          
          let secondSkipIndex = (skipIndex === points.length-1) ? skipIndex-1 : skipIndex +1;
          //drawLines.push(drawArc(points[secondSkipIndex], 2, 0, 360));

          
          let secondInverseSkipIndex = (skipIndex === points.length-1) ? inverseSkipIndex -1 : inverseSkipIndex + 1;
          //drawLines.push(drawArc(points[secondInverseSkipIndex], 2, 0, 360));
          ...
          for (let f = 0; f < facetlines.length; f++) {
            ...
            else if (f === skipIndex+1) { facetpoints.push(facetlines[f][0]);}
            else {
            let l = facetlines[f];
            let newFacetPoint  = [l[1][0]-v[0], l[1][1]-v[1]];
            templines.push([l[0], newFacetPoint]);
            facetpoints.push(newFacetPoint);
            if (f === inverseSkipIndex) {
              beforeSkipIndex = (inverseSkipIndex < secondInverseSkipIndex) ? inverseSkipIndex -1 : inverseSkipIndex + 1;
              let l1 = facetlines[beforeSkipIndex];
              // drawLines.push(drawArc([l1[1][0]-v[0], l1[1][1]-v[1]], 2, 0, 360));
              // drawLines.push(drawArc([l1[0][0], l1[0][1]], 2, 0, 360));
              // drawLines.push(drawArc(newFacetPoint, 2, 0, 360));

              polygonsToHatch.push({p:[l[0], newFacetPoint, [l1[1][0]-v[0], l1[1][1]-v[1]], [l1[0][0], l1[0][1]]], h:4});

              let l2 = facetlines[secondInverseSkipIndex];
              polygonsToHatch.push({p:[l[0], newFacetPoint, [l2[1][0]-v[0], l2[1][1]-v[1]], [l2[0][0], l2[0][1]]], h:2});
            }
            if (f === secondInverseSkipIndex) {
              ...
              let l1 = facetlines[afterSkipIndex];
              // drawLines.push(drawArc([l1[1][0]-v[0], l1[1][1]-v[1]], 2, 0, 360));
              // drawLines.push(drawArc([l1[0][0], l1[0][1]], 2, 0, 360));
              // drawLines.push(drawArc(newFacetPoint, 2, 0, 360));
              polygonsToHatch.push({p:[newFacetPoint, [l1[1][0]-v[0], l1[1][1]-v[1]], [l1[0][0], l1[0][1]], l[0]], h:4});
            }
            }
          }
        }
...
const sketch = ({ width, height }) => {
  return ({ context, width, height, units }) => {
    ...
    const draw = (origin, w, h) => {
      ...
      drawGemstone({c:center,r:(w - margin)/2,m:margin}, {o:origin})
