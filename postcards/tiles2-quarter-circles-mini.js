...
const createGrid = (columns, rows, w, h, marginX) => {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < columns; c++) {
      ...
      let lines = random.value() < config.lines;
      let mini = !lines && random.value() < config.mini;
...
const drawTile = (x, y, side, rnd, mini = {}, padding = 0) => {
  ...
  switch (corner) {
    ...
    case 2:
      cx = x + side - padding;
      cy = y + side - padding;
      (ocx = x + padding), (ocy = y + padding), (startAngle = 180);
      ostartAngle = 0;
      p0 = twoCorner;
      p1 = oneCorner;
      (p2 = threeCorner),
    ...
    case 3:
      cx = x + padding;
      cy = y + side - padding;
      (ocx = x + side - padding), (ocy = y + padding), (startAngle = 270);
      ostartAngle = 90;
      p0 = threeCorner;
      p1 = twoCorner;
      (p2 = zeroCorner),
  ...
  if (rnd.mini) {
    ...
    if (rnd.lines) {
      ...
      for (let s = 1; s < divide; s++) {
        ...
        if (!i || i.length === 0) {
          result.push(
            createArcPath([ocx, ocy], s * step, ostartAngle, ostartAngle + 90)
          );
        } else {
          i.forEach((element) => {
            result.push(
              createArcPath([ocx, ocy], s * step, ostartAngle, element[1])
            );
            result.push(
              createArcPath([ocx, ocy], s * step, element[0], ostartAngle - 270)
            );
          });
        }
      }
...
const sketch = ({ width, height }) => {
  ...
  //tile grid
  const countX = config.countX;
  const countY = Math.floor(cardHeight / Math.floor(cardWidth / countX));
  const margin = cardWidth * config.margin;
  ...
  return ({ context, width, height, units }) => {
    ...
    const draw = (origin, w, h, opts) => {
      ...
      for (let row = 0; row < countY; row++) {
        for (let column = 0; column < countX; column++) {
          ...
          const tile = ({ border, draw01, draw12, draw23, draw30 } =
            grid[row][column]);
    ...
    postcards.drawColumnsRowsPortrait(
      draw,
      width,
      height,
      postcardGrid.columns,
      postcardGrid.rows
    );
