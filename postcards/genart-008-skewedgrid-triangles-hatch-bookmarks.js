...
const createGrid = (count, width, height) => {
  const countX = count;
  const countY = countX * 2 //Math.floor(countX / width * height);
  ...
  for (let x = 0; x < countX; x++) {
    ...
    for (let y = 0; y < countY; y++) {
      ...
      points.push({
        ...
        noise,
        hatch1: noise < 0.75 && noise > 0.55,
        hatch2: noise < 0.29 && noise > 0.22
      });
    }
    grid.push(points);
  }
...
const sketch = ({ width, height }) => {
  ...
  return ({ context, width, height, units }) => {
    ...
    const draw = (origin, w, h) => {
      ...
      let grid = createGrid(7, width, height);
        ...
        for (let column = 0; column < grid[row].length; column++) {
          ...
          if (!isLastColumn && !isLastRow) {
            ...
            if ((n1+n2+n3) > 1) {
              ...
              let space = dynspace * (n1+n2+n3) * 1.5; //spacing;
            ...
            if ((n1+n4+n3) > 1) {
              ...
              let space = dynspace * (n1+n4+n3) * 1.5; //spacing;
